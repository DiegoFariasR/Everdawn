...
public class DamageResistanceTests
{
    ...
    private static BattleUnit MakeTarget(IReadOnlyDictionary<DamageType, int>? resistances = null) =>
        new("target", "Target", "enemy", Level: 1, Str: 100, Wis: 0, Agi: 50, Resistances: resistances);

    private static DamageResult Compute(BattleUnit actor, BattleUnit target, DamageType type, int seed = 0) =>
        DamageCalc.Compute(actor, target, type, 1.0, 1.0, new Random(seed));

    // ── DamageResult structure ────────────────────────────────────────────

    [Fact]
    public void Result_RecordsDamageType()
    {
        var result = Compute(MakeActor(), MakeTarget(), DamageType.Magical);
    ...
    {
        var result = Compute(MakeActor(str: 100), MakeTarget(), DamageType.Physical);
    ...
    {
        var target = MakeTarget(new Dictionary<DamageType, int> { [DamageType.Physical] = 50 });
        var result = Compute(MakeActor(str: 100), target, DamageType.Physical);
    ...
    {
        var target = MakeTarget(new Dictionary<DamageType, int> { [DamageType.Physical] = 100 });
        var result = Compute(MakeActor(str: 100), target, DamageType.Physical);
    ...
    [Fact]
    public void PhysicalResistance_DoesNotAffectMagicalDamage()
    {
        var target = MakeTarget(new Dictionary<DamageType, int> { [DamageType.Physical] = 100 });
        var result = Compute(MakeActor(wis: 100), target, DamageType.Magical);
        Assert.Equal(result.RawDamage, result.FinalDamage);
        Assert.True(result.FinalDamage > 0);
    }

    [Fact]
    public void MagicalResistance_DoesNotAffectPhysicalDamage()
    {
        var target = MakeTarget(new Dictionary<DamageType, int> { [DamageType.Magical] = 100 });
        var result = Compute(MakeActor(str: 100), target, DamageType.Physical);
        Assert.Equal(result.RawDamage, result.FinalDamage);
        Assert.True(result.FinalDamage > 0);
    }
    ...
    public void PhysicalType_UsesPhysAttack()
    {
        ...
        var physResult = Compute(actor, MakeTarget(), DamageType.Physical);
        var magResult = Compute(actor, MakeTarget(), DamageType.Magical);
        Assert.True(magResult.RawDamage > physResult.RawDamage);
    }

    [Fact]
    public void MagicalType_UsesMagicAttack()
    {
        var actor = MakeActor(str: 100, wis: 0);
        var physResult = Compute(actor, MakeTarget(), DamageType.Physical);
        var magResult = Compute(actor, MakeTarget(), DamageType.Magical);
        Assert.True(physResult.RawDamage > magResult.RawDamage);
    }
    ...
    {
        var result = Compute(MakeActor(), MakeTarget(), DamageType.Physical);
    ...
    public void Pipeline_BaseStep_ValueBeforeIsBaseAttackStat()
    {
        ...
        var result = Compute(actor, MakeTarget(), DamageType.Physical);
    ...
    {
        var unit = MakeTarget();
        Assert.Equal(0, unit.GetResistance(DamageType.Physical));
        Assert.Equal(0, unit.GetResistance(DamageType.Magical));
    }

    [Fact]
    public void NaturalDamageType_IsMagicalWhenWisHigher()
    {
        var mage = MakeActor(str: 10, wis: 100);
        Assert.Equal(DamageType.Magical, mage.NaturalDamageType);
    }

    [Fact]
    public void NaturalDamageType_IsPhysicalWhenStrHigher()
    {
        var warrior = MakeActor(str: 100, wis: 10);
        Assert.Equal(DamageType.Physical, warrior.NaturalDamageType);
    }
}

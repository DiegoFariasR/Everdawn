...
### Requirement Types
- [x] **PermittedTraits** — the unit must have at least one of the specified `BattleTrait` values (e.g. `ManaUser` for spells)
...
- Equipment type and permitted traits are authored in YAML (`permittedEquipmentTypes` as a list, `permittedTraits` as a list) and on `BattleUnit` (`equipmentType`).
